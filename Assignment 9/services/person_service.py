...
class PersonService:
    ...
    def delete_person_by_id(self, person_id, record_undo=True, record_redo=False, as_redo=False):
        ...
        if person_id <= 0:
            raise PersonIDException("Error! The person ID must be a positive integer.")
        person_to_remove = self.find_person_by_id(person_id)
    ...
    def get_name_of_person_by_id(self, person_id):
        ...
        found_person = self.find_person_by_id(person_id)
    ...
    def get_all_names(self):
        ...
        """
        return [person.name for person in self.__person_repository]

    def get_all_phone_numbers(self):
        """
        Returns all the phone numbers currently registered in the person repository
        """
        return [person.phone_number for person in self.__person_repository]
    ...
    def search_by_name(self, name):
        ...
        return [person for person in all_persons if name.lower().strip() in person.name.lower().strip()]
    ...
    def search_by_phone_number(self, phone_number):
        ...
        return [person for person in all_persons if phone_number in person.phone_number.replace(' ', '')]
