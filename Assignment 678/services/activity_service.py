...
class ActivityService:
    ...
    def delete_activity_by_id(self, activity_id, record_undo=True, record_redo=False, as_redo=False):
        ...
        activity_to_remove = self.find_activity_by_id(activity_id)
    ...
    def add_persons_by_id_to_activity(self, activity_id, person_ids, used_to_init_new_activity=False, record_undo=True,
        ...
        passed_valid, not_passed_valid = list(set(passed_valid)), list(set(not_passed_valid))  # Ignore duplicate IDs
        activity_to_add_to = self.find_activity_by_id(activity_id)
        ...
        # todo: change here
        # for id_ in passed_valid:
        #     activity_to_add_to.add_person_id(id_)
    ...
    def remove_persons_by_id_from_activity(self, activity_id, person_ids, record_undo=True,
        ...
        passed_valid, not_passed_valid = self.__persons_id_validator.validate(person_ids)
        passed_valid, not_passed_valid = list(set(passed_valid)), list(set(not_passed_valid))  # Ignore duplicate IDs
        activity_to_remove_from = self.find_activity_by_id(activity_id)
        ...
        # todo: change here
        # for id_ in passed_valid:
        #     activity_to_remove_from.remove_person_id(id_)
        new_persons_ids = []
    ...
    def get_all_activities_of_person_id(self, person_id):
        ...
        return [activity for activity in all_activities
                if ActivityService.check_person_id_in_activity(activity, person_id)]
    ...
    def update_activity_start_date_time(self, activity_id, new_start_date_time, record_undo=True,
        ...
        activity_to_update = self.find_activity_by_id(activity_id)
        ...
        # todo: change here
        # activity_to_update.change_start_date(new_datetime)
    ...
    def update_activity_end_date_time(self, activity_id, new_end_date_time, record_undo=True,
        ...
        # todo: change here
        # activity_to_update.change_end_date(new_datetime)
    ...
    def update_activity_description(self, activity_id, new_description, record_undo=True, record_redo=False,
        ...
        # todo: change here
        # activity_to_update.change_description(new_description)
    ...
    def search_by_datetime(self, search_datetime):
        ...
        if helper_combined_datetimes.year == now_datetime.year and \
            ...
            found_activities = [activity for activity in all_activities if
                                datetime.time(activity.start_hour, activity.start_minute) <= search_time
                                <= datetime.time(activity.end_hour, activity.end_minute)]
            ...
            found_activities = [activity for activity in all_activities if
                                datetime.date(activity.start_year, activity.start_month, activity.start_day) <=
                                search_date <=
                                datetime.date(activity.end_year, activity.end_month, activity.end_day)]
        else:
            ...
            found_activities = [activity for activity in all_activities if
                                datetime.datetime(activity.start_year, activity.start_month, activity.start_day,
                                                  activity.start_hour, activity.start_minute) <=
                                search_date_and_time <=
                                datetime.datetime(activity.end_year, activity.end_month, activity.end_day,
                                                  activity.end_hour, activity.end_minute)]
    ...
    def sorted_activities_in_given_date(self, input_date):
        ...
        activities_on_date = [activity for activity in self.get_all_activities() if
                              datetime.date(activity.start_year, activity.start_month, activity.start_day) <=
                              search_date <=
                              datetime.date(activity.end_year, activity.end_month, activity.end_day)]
        sorted_activities = sorted(activities_on_date, key=lambda activity: (activity.start_year, activity.start_month,
                                                                             activity.start_day, activity.start_hour,
                                                                             activity.start_minute))
    ...
    def search_person_by_id_or_name(self, person_info):
        ...
        all_persons = self.__person_repository.entities
    ...
    def activities_with_given_person(self, person_info):
        """
        Returns all activities to which a given person will participate in.
        ...
        all_activities = self.get_all_activities()
        person_activities = [activity for activity in all_activities if found_person.id in activity.persons_id]
        sorted_person_activities = sorted(person_activities, key=lambda activity: (activity.start_year,
                                                                                   activity.start_month,
                                                                                   activity.start_day,
                                                                                   activity.start_hour,
                                                                                   activity.start_minute))
        return sorted_person_activities
    ...
    def busiest_days_person(self, person_info):
        ...
        for date_nr, (date, start_time, end_time, total_minutes) in enumerate(zip(sorted_dates, sorted_start_times,
            ...
            for index in range(len(start_time)):
                start_free_time = datetime.datetime.combine(datetime.date.min, end_time[index]) + \
                                  datetime.timedelta(minutes=1)
                if start_time[index] != datetime.time(23, 59):
                    end_free_time = datetime.datetime.combine(datetime.date.min, start_time[index]) - \
                                    datetime.timedelta(minutes=1)
                else:
                    end_free_time = datetime.datetime.combine(datetime.date.min, start_time[index])
    ...
    def add_person_to_activities(self, person_id, activity_ids, record_undo=True, record_redo=False, as_redo=False):
        ...
        person_to_add = self.__person_repository.find_by_id(person_id)
        ...
        for activity in all_activities:
            if activity.id in add_to and person_id not in activity.persons_id:
                # todo: change here
                # activity.persons_id.append(person_id)
    ...
    def delete_person_from_activities(self, person_id, activity_ids="", record_undo=True, record_redo=False,
        ...
        person_to_delete = self.__person_repository.find_by_id(person_id)
